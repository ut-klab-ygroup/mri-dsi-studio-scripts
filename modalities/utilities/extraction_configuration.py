...
def update_configuration(config, project_name, subject_name, condition):
    """Update the configuration with the processed subject, grouping by condition."""
    if project_name not in config['projects']:
    ...
    # Add subject to 'subjects_processed'
    config['projects'][project_name]['subjects_processed'][subject_name] = "processed"

    # Add subject under the appropriate condition
    if condition not in config['projects'][project_name]['conditions']:
        config['projects'][project_name]['conditions'][condition] = []

    if subject_name not in config['projects'][project_name]['conditions'][condition]:
        config['projects'][project_name]['conditions'][condition].append(subject_name)

    # Capture the date of the last run
    config['projects'][project_name]['last_run'] = datetime.now().strftime("%Y-%m-%d")
